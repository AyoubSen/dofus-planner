...
export default defineEventHandler(async (event) => {
  try {
    ...
    return await dofusdbFetch("/spell-levels", {
      lang: "fr",
      ...query,
    });
  } catch (error: any) {
    console.error("Error fetching DofusDB spell-levels:", error);
    throw createError({
      statusCode: error?.statusCode || 500,
      statusMessage: error?.statusMessage || "Failed to fetch spell-levels",
    });
  }
});
